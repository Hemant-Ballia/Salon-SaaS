...
export async function registerCustomerApi(data: RegisterCustomerData): Promise<{ user: User }> {
  const response = await apiClient.post<ApiResponse<{ user: User }>>("/auth/register/customer", data);
